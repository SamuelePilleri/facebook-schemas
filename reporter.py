...
BASE_URI = 'https://samuelepilleri.github.io/facebook-schemas/'
...
def main(args):
	...
	with zipfile.ZipFile(args[0], 'r') as dump:
		...
		missings = frozenset([
			...
			if not re.match(r'messages/.+/message\.json', file) and not file.startswith('photos_and_videos/album/')
		])
		...
		for file in filter(lambda file: re.match(r'messages/.+/message\.json', file), files):
			with dump.open(file, 'r') as instance:
				validate(file, load_facebook_json(instance), cache[BASE_URI + 'messages/message.json'])
		print('')
		...
		for file in filter(lambda file: file.startswith('photos_and_videos/album/'), files):
			with dump.open(file, 'r') as instance:
				validate(file, load_facebook_json(instance), cache[BASE_URI + 'photos_and_videos/album/album.json'])
